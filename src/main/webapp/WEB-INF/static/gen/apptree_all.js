...
$(document).ready(function(){
    window.ajax_dev_id = null;
    window.ajax_udev_id = null;
    window.ajax_contact_id = null;
    window.init_time = new Date();
    var help_dom = $.parseStr(
        '<div style="%s"><i class="icon-question"></i>&nbsp;<a href="%s" target="_blank">help</a></div>',
        'float: right; color: green',
        'http://wiki.corp.qunar.com/pages/viewpage.action?pageId=52799461'
    );
    $("#breadcrumbs").append(help_dom);
    showTips = $.createMsgTipsFn();
    $.setObjectFull($("#tree_panel"), 124, 12, 0);
    loadAppTree();
    bindSelectDevEvent();
});

function showNodeFromUrlParams(url_parmas){
    var url_p = url_parmas.replace(/\s/g, '');
    var params = url_p.split('&amp;');
    ...
    for(var i=0; i<p_len; i++){
        ...
        if(l[0] == 'hostname'){
            showNodeInfoByHostname(l[1]);
            return
        }else if(l[0] == 'tree_path'){
            showNodeInfoByTreePath(l[1]);
            return
        }
    }
}
...
function showNodeInfoByTreePath(tree_path){
    var list = tree_path.split('.');
    var list_len = list.length;
    if(list_len <= 0){
        return
    }
    var name = list[list_len - 1];
    $.ajax({
        type: "GET",
        url: 'search_paths',
        dataType: "json",
        ...
        success: function(resp) {
            ...
            var s_node = zTree.getNodeByParam('id', tree_id)
            if (s_node){
                zTree.selectNode(s_node)
                showAppDetail('', 'treeDemo', s_node)
                showDevCountAbove(s_node)
            }
...
function showNodeInfoByHostname(hostname){
    if(! hostname){
        return
    }
    $.ajax({
        type: "GET",
        url: 'get_device_match',
        dataType: "json",
        data: {'start_with': hostname},
        success: function(resp) {
            var data = resp.data || {};
            var d_len = data.length;
            if(d_len <=0 ){
            ...
            $.ajax({
                type: "GET",
                url: "get_treeid_by_devid",
                dataType: "json",
                data: {dev_id: dev_id},
                success: function(resp) {
                    var suc = resp.success;
                    if(suc){
                        var tree_id = resp.data;
                        var s_node = zTree.getNodeByParam('id', tree_id);
                        if (s_node){
                            zTree.selectNode(s_node)
                            showAppDetail(event, 'treeDemo', s_node)
                            showDevCountAbove(s_node)
                        }
                    }
                }
...
function bindSelectDevEvent(){
    $("#device_sel").autocomplete({
        source: function(request, response) {
            var name = request.term.replace(/(^[\s;]*)|([\s;]*$)/g, "");
            var list = name.split(/\s+/);
            if(list.length > 0){
                if(list[0] == '~'){
                    if (list.length > 1){
                        if (list[1].length < 2){return}
                        var url = 'search_paths';
                        var data = {'name': list[1]};
                        var label= 'path';
                        var value = 'path';
                        var param = 'tree_id'
                        var type = 'node';
                    ...
                    var url = 'get_device_match';
                    var data = {'start_with': list[0]};
                    var label= 'name';
                    var value = 'name';
                    var param = 'id';
                    var type = 'dev';
        ...
        }else if(ui.item.type == 'node'){
            var tree_id = ui.item.param;
            var s_node = zTree.getNodeByParam('id', tree_id)
            if (s_node){
                zTree.selectNode(s_node)
                showAppDetail(event, 'treeDemo', s_node)
...
function showDevCountAbove(node){
    if(zTree.getNodeByParam('id', node.parent_id)){
    showDeviceCount('', 'treeDemo', zTree.getNodeByParam('id', node.parent_id))
    }
    if(node.parent_id > 1){
        showDevCountAbove(zTree.getNodeByParam('id', node.parent_id))
    }
}
...
function showRMenu(type, x, y) {
    $("#rMenu ul").show();
    var rMenu_height = $("#rMenu ul").height()
    win_height = $(window).height()
    y = Math.min((win_height - rMenu_height -5), y)
...
function showAppDetail(event, treeId, treeNode){
    window.contact_p = false
    window.super_p = false
    window.device_p = false
    if (!treeNode || treeNode.id==1){
        $("#app_detail_panel").hide();
        return;
    }
    $("#app_detail_panel").show()
    showTreePath(treeNode.id)
    $.ajax({
        url: 'get_node_permi',
        dataType: 'json',
        contentType: "application/x-www-form-urlencoded",
        type: 'POST',
        data: {tree_id: treeNode.id},
        success: function(resp){
            if(resp.success){
                var permi = resp.data;
                contact_p = permi['contact_p'];
                super_p = permi['super_p'];
                device_p = permi['device_p'];
                showContactsList(treeNode.id, permi['contact_p'], permi['super_p']);
                showComment(treeNode.id, permi['device_p']);
                showBindDevicesList(treeNode.id, permi['device_p']);
                showUnbindDevicesList(treeNode.id, true, permi['device_p']);
            }else{
                contact_p = false;
                super_p = false;
                device_p = false;
                showContactsList(treeNode.id, false, false);
                showComment(treeNode.id, false);
                showBindDevicesList(treeNode.id, false);
                showUnbindDevicesList(treeNode.id, true, false);
...
function showTreePath(tree_id){
    ...
    $.ajax({
        ...
        data: params,
        success: function(resp){
            var tree_path = resp.data
            $('#tree_path').html(tree_path)
        },
        error: function(){
            showTips(3, '网络错误')
        }
...
function addTreeNode() {
    ...
    var form = '<form id="add_tree_node">';
    form += '<div><label for="app_name_in">应用名称：</label>';
    form += '<input id="app_name_in" name="app_name_in" style="width:240px"></div>';
    form += '<div><label for="app_name_in">应用负责人：</label>';
    form += '<input id="contact_list_select" name="rtx_ids" style="width:240px"></div></form>'
    $("#add_contact_dialog").append(form);
    $("#contact_list_select").autocomplete({
        source: function( request, response ) {
            var term = request.term.split(';');
            var term_len = term.length;
            con_str = [];
            if(term_len>1){
                for (var i=0; i<term_len-1; i++){
                    con_str.push(term[i])
                }
            }
            if(term[term_len - 1].length>=2){
                $.ajax({
                    type: "GET",
                    url: "get_contact_list",
                    dataType: "json",
                    data: {start_with: term[term_len - 1]},
                    success: function(resp) {
                        response($.map(resp.data, function(item){
                            return {
                                label: item.username + '（' + item.username_cn + '）',
                                value: item.username,
                                rtx_id: item.username
                            }
                        }));
                    }
                });
        ...
        minLength: 2,
        select: function(event, ui) {
            con_str.push(ui.item.value);
            setTimeout(function(){
                $("#contact_list_select").val(con_str.join(';') + ';');
                var t=$("#contact_list_select").val();
                $("#contact_list_select").val("").focus().val(t);
            }, 10)
        },
        open: function() {
            $(this ).removeClass("ui-corner-all").addClass("ui-corner-top");
    ...
    var title_html_str = '<div class="dialog_title">增加新应用</div>'
    var add_tree_node_form = createTreeNodeNameFormValidate($("#add_tree_node"))
    var dialog = $("#add_contact_dialog").removeClass('hide').dialog({
        modal: true,
        width: 380,
        resizable: true,
        ...
        buttons: [
            {
                ...
                click: function(){
                    if (! $("#add_tree_node").valid()){
                        return
                    }
                    node_name = $('#app_name_in').val().toLowerCase()
                    params = {
                        'node_name': node_name,
                        'parent_id': node.id,
                        'rtx_ids': $("#contact_list_select").val().replace(/(^[\s;]*)|([\s;]*$)/g, "").replace(/;/g, ',')
                    }
                    $.ajax({
                        ...
                        success: function(resp){
                            ...
                            var data = resp.data
                            if(is_succ == true){
                                zTree.addNodes(node, {'id': data.tree_id, 'parent_id': data.parent_id, 'name': node_name});
                                $("#add_contact_dialog").dialog('close')
                                showTips(1, '新增应用成功')
                                var s_node = zTree.getNodeByParam('id', data.tree_id)
                                if (s_node){
                                    zTree.selectNode(s_node)
                                    showAppDetail('', 'treeDemo', s_node)
                                    showDevCountAbove(s_node)
            ...
            close: function(){
                $("#add_contact_dialog").empty()
                $("#add_contact_dialog").dialog('destroy')
            }
    });
}
...
function showApiKey() {
    ...
    $.ajax({
        ...
        success: function(resp){
            ...
            var is_succ = resp.success
            if(is_succ == true){
                var title_html_str = '<div class="dialog_title">';
                title_html_str += 'APIKEY';
                title_html_str += '</div>';

                var confirm_msg_div = '<div class="alert alert-info bigger-110">';
                confirm_msg_div += data.api_key;
                confirm_msg_div += '</div>';
                $("#add_contact_dialog").append(confirm_msg_div)
...
function renameTreeNode() {
    ...
    $("#add_contact_dialog").append('<form id="edit_tree_node"><div><label for="app_name_in">应用新名称：</label><input id="app_name_in" name="app_name_in" style="width:240px"></div></form>')
    var edit_tree_node_form = createTreeNodeNameFormValidate($("#edit_tree_node"))
    title_html_str = '<div class="dialog_title">修改应用名称</div>'
    $('#app_name_in').val(node.name)
    var dialog = $("#add_contact_dialog").removeClass('hide').dialog({
        modal: true,
        width: 380,
        resizable: true,
...
function removeTreeNode() {
    ...
    if (nodes && nodes.length>0) {
        var node = nodes[0]
        var confirm_msg_div = '<div class="alert alert-info bigger-110">'
        confirm_msg_div += '确认要删除该应用吗？'
        confirm_msg_div += '</div>'
        $("#add_contact_dialog").append(confirm_msg_div)
    }

    title_html_str = '<div class="dialog_title">删除应用</div>'
    var dialog = $("#add_contact_dialog").removeClass('hide').dialog({
        modal: true,
        width: 280,
        resizable: true,
...
function loadAppTree(){
    var mark_obj = $(".page-content").mark({})
    var setting = {
        view: {
        ...
        edit: {
            drag: {
                ...
                inner: function(treeId, nodes, targetNode){
                    if (!targetNode|| targetNode.id==1){return false}
                    return true
                },
                isCopy: false,
                isMove: true,
        ...
        callback: {
            ...
            onClick: showAppDetail,
            beforeDrag: function(treeId, nodes){
                if (nodes[0].parent_id<=1){return false}
                return checkNodesNameValid(nodes)
            },
            beforeDrop: editPid,
            onExpand: showDeviceCount
        }
    };
    $.ajax({
        url: 'get_app_tree',
        ...
        success: function(resp){
            var data = resp.data;
            if (data.length == 0){
                showTips(2, '你没有查看应用树权限')
                return
            }
            $.fn.zTree.init($("#treeDemo"), setting, data);
            window.zTree = $.fn.zTree.getZTreeObj("treeDemo");
            window.rMenu = $("#rMenu");
            zTree.expandNode(zTree.getNodeByParam('parent_id', 0), true, false, true, true);
            mark_obj.hide();
            showNodeFromUrlParams(url_parmas);
        }
    })
}

function checkNodesNameValid(nodes, name){
    var str = name || ''
    ...
    if(len>0){
        for(var i=0; i<len; i++){
            if(str == ''){
                ...
                var s = str + ' > ' + nodes[i].name
            }
            var user_list_regex = new RegExp(/^([a-zA-Z])([a-zA-Z0-9_])*([a-zA-Z0-9])$/)
            if(!user_list_regex.test(nodes[i].name)){
                showTips(2, "节点名称必须以字母开头,不可下划线结尾: " + s)
                return false
            }else{
                if('children' in nodes[i]){
                    if(! checkNodesNameValid(nodes[i].children, s)){
                        return false
                    }
                }
            }
...
function showDeviceCount(e, treeId, treeNode){
    $.ajax({
        url: 'get_dev_count_belongs_tree',
        async: false,
        method: 'GET',
        type: 'json',
        data: {tree_id: treeNode.id},
        success: function(resp){
            var dcs = resp.data;
            var d_len = dcs.length
            if (d_len == 0){
                return
            }
            for (var i=0; i<d_len; i++){
                var c_node = zTree.getNodeByParam('id', dcs[i].tree_id)
                $("#" + c_node.tId + "_span").html(c_node.name +
                    '<span>&nbsp;</span><span style="padding:0px 4px; font-size: 12px;color:ForestGreen">' 
                + dcs[i].count + '</span>')
...
function editPid(treeId, nodes, tarNode){
    var tree_id = nodes[0].id;
    var parent_id = tarNode.id;
    var node = nodes[0]
    var from_params = {'tree_id': tree_id}
    var to_params = {'tree_id': parent_id}
    $.ajax({
        url: 'get_parent_tree',
        dataType: 'json',
        contentType: "application/x-www-form-urlencoded",
        ...
        success: function(resp){
            ...
            $.ajax({
                ...
                data: to_params,
                success: function(resp){
                    var tree_path_to = resp.data
                    var confirm_msg_div = '<div class="alert alert-info bigger-110">'
                    confirm_msg_div += '<span>从: ' + tree_path_from + '</span><br>'
                    confirm_msg_div += '<span>到: ' + tree_path_to + '</span>'
                    confirm_msg_div += '</div>'
                    $("#add_contact_dialog").append(confirm_msg_div)
                }
            })
    ...
    title_html_str = '<div class="dialog_title">移动' + nodes[0].name + '</div>'
    var dialog = $("#add_contact_dialog").removeClass('hide').dialog({
        modal: true,
        width: 450,
        resizable: true,
        title: title_html_str,
        title_html: true,
        position: { my: "left top", at: "right top", of: '#device_sel'},
        buttons: [
            {
                text: "确认",
                "class" : "btn btn-danger btn-xs",
                click: function(){
                    ...
                    $.ajax({
                        ...
                        success: function(resp){
                            var is_succ = resp.success
                            if(is_succ == true){
                                zTree.moveNode(tarNode, nodes[0], 'inner')
                                $("#add_contact_dialog").dialog('close')
                                showTips(1, '移动节点成功')
                                setTimeout(function(){
                                    $("#app_detail_panel").hide()
                                    showDevCountAbove(zTree.getNodeByParam('id', nodes[0].parent_id))
                                    showDevCountAbove(zTree.getNodeByParam('id', parent_id))
                                }, 10);
                            }else{
                                $("#add_contact_dialog").dialog('close')
                                showTips(2, resp.msg)
...
function showComment(tree_id, bShowToolBar){
    bShowToolBar = (typeof(bShowToolBar)=='undefined') ? true: bShowToolBar;
    $("#comment_panel").empty();
    $("#comment_panel").append($.parseStr(
        '<div class="%s" style="%s">备注<button id="edit_comment" class="%s" style="%s">编辑</button></div>',
        'table-header',
        'font-size:14px; font-weight: bold',
        'right btn btn-xs btn-success',
        'font-size:14px; font-weight: bold; float: right; margin: 4px'
    ))
    $("#comment_panel").append('<textarea id="comment_textarea" style="width: 100%; resize: none; border: 0px" readonly></textarea>');
    $.setObjectFull($("#comment_textarea"), 146, 6, 38);
    $.ajax({
        url: 'get_comment',
        dataType: 'json',
        contentType: "application/x-www-form-urlencoded",
        ...
        success: function(resp){
            if(resp.data){
                var d = resp.data;
                $("#comment_textarea").val(d.tree_comment || '');
            }
        }
    });
    if(bShowToolBar){
        $("#edit_comment").unbind('click');
        $("#edit_comment").click({tree_id: tree_id}, function(e){
            if($("#edit_comment").hasClass('btn-success')){
                $("#comment_textarea").attr("readonly", false).focus();
                $("#edit_comment").removeClass('btn-success').addClass('btn-warning').html("保存");
            }else{
                $.ajax({
                    url: 'save_comment',
                    dataType: 'json',
                    ...
                    type: 'POST',
                    data: {
                        tree_id: e.data.tree_id,
                        tree_comment: ($("#comment_textarea").val() || '')
                    },
                    success: function(resp){
                        if(resp.success){
                            showTips(1, resp.msg);
                            showComment(e.data.tree_id);
                        }else{
                            showTips(2, resp.msg);
                        }
                    },
                ...
                $("#edit_comment").removeClass('btn-warning').addClass('btn-success').html("编辑");
            }
        })
...
function showContactsList(tree_id, bShowToolBar, bSuper){
    bShowToolBar = (typeof(bShowToolBar)=='undefined') ? true: bShowToolBar
    bSuper = (typeof(bSuper)=='undefined') ? false: bSuper
    disabled = bSuper ? false: true
    now_time = new Date()
    $("#contact_panel").dataTablePanel({
        headerName: '应用负责人列表',
        bFull: true,
        rowspan: 's',
        rest_h: 146,
        ...
        bMutiSelect: true,
        aoColumns: [
            {"sTitle": "rtx_id", "sWidth": "100px", "bVisible": false},
            {"sTitle": "姓名"},
            {"sTitle": "电话", "sWidth": "100px", "sClass": "center"},
            {"sTitle": "审批", "sWidth": "50px", "sClass": "center"}
        ],
        aDataSet: {tree_id: tree_id},
        formatAaData: function(resp, fieldCol){
            var aaData = []
            ...
            if (raw_data_len > 0){
                for(var i=0; i<raw_data_len; i++){
                    var row_d = []
                    row_d.push(raw_data[i]['username'])
                    row_d.push(raw_data[i]['username'] + '（' + raw_data[i]['username_cn'] + '）')
                    row_d.push(raw_data[i]['phone'])
                    if (raw_data[i]['permission']){
                        var per_str = '<span style="color: green; font-weight: 700">Y</span>' 
                    }else{
                        var per_str = '<span style="color: red; font-weight: 700">N</span>' 
                    }
                    row_d.push(per_str)
                    aaData.push(row_d)
                }
        ...
        ajax_url: "node/association/get_contacts?_dc=" + now_time.getTime(),
        method: 'GET',
        bShowToolBar: bShowToolBar,
        toolBar: [{
            id: 'authority_manage',
            name: '<i class="icon-key"></i>',
            type: 'success',
            floated: 'right',
            disabled: disabled, 
            is_menu: true,
            menus: [{
                ...
                name: '加审批权',
                type: 'success',
                clickFn: function(otable, rowDatas, otherParams){
                    var row_datas_len = rowDatas.length
                    if(row_datas_len == 0){
                        showTips(2, '未选择应用负责人')
                    }else{
                        var rtx_list = []
                        for(var i=0; i<row_datas_len; i++){
                            rtx_list.push(rowDatas[i][0])
                        }
                        var rtx_ids = rtx_list.join(',')
                        editApprove(otherParams.tree_id, rtx_ids, 1)
                    }
                }
            },{
                id: 'remove_approve',
                ...
                clickFn: function(otable, rowDatas, otherParams){
                    ...
                    if(row_datas_len == 0){
                        ...
                        var rtx_ids = rtx_list.join(',')
                        editApprove(otherParams.tree_id, rtx_ids, 0)
                    }
                }
            }]
        },{
            id: 'change_duty_order',
            name: '<div class="rotate-90"><i class="icon-exchange"></i></div>',
            type: 'warning',
            floated: 'right',
            is_menu: true,
            menus:[{
                ...
                clickFn: function(otable, rowDatas, otherParams){
                    ...
                    if(row_datas_len == 0){
                        ...
                        var rtx_ids = rtx_list.join(',')
                        changeContactDutyOrder(otherParams.tree_id, rtx_ids, 'up')
                    }
                }
            },{
                id: "down_duty_order",
                ...
                clickFn: function(otable, rowDatas, otherParams){
                    ...
                    if(row_datas_len == 0){
                        ...
                        var rtx_ids = rtx_list.join(',')
                        changeContactDutyOrder(otherParams.tree_id, rtx_ids, 'down')
                    }
                }
            }]
        },{
            id: 'remove_contact',
            name: '<i class="icon-minus"></i>',
            type: 'danger',
            floated: 'right',
            title: "删除联系人",
            clickFn: function(otable, rowDatas, otherParams){
                ...
                if(row_datas_len == 0){
                    ...
                    var rtx_ids = rtx_list.join(',')
                    removeContact(otherParams.tree_id, rtx_ids)
                }
            }
        },{
            id: 'add_contact',
            name: '<i class="icon-plus"></i>',
            type: 'primary',
            floated: 'right',
            title: "新增联系人",
            clickFn: function(otable, rowDatas, otherParams){
                addContactDialog(otherParams.tree_id)
            }
        },{
            ...
            title: "全选 / 全不选",
            clickFn: function(otable, rowDatas, otherParams){
                if($("#select_all_contacts").find('.icon-stop').length==0){
                    otable.$("tr").each(function(){
                        $(this).removeClass('row_selected'); 
                    })
                    $("#select_all_contacts").html('<i class="icon-stop"></i>')
                }else{
                    otable.$("tr").each(function(){
                        $(this).addClass('row_selected'); 
                    })
                    $("#select_all_contacts").html('<i class="icon-check"></i>')
                }
            }
        }]
    })
...
function removeContact(tree_id, rtx_ids){
    params = {
        'tree_id': tree_id,
        'rtx_ids': rtx_ids
    }
    $.ajax({
        url: 'node/association/unbind_contacts',
        dataType: 'json',
        contentType: 'application/x-www-form-urlencoded',
        type: 'POST',
        data: params,
        success: function(resp){
            var is_succ = resp.success
            if(is_succ == true){
                showContactsList(tree_id, contact_p, super_p)
                showTips(1, '解除负责人成功')
            }else{
                showTips(2, resp.msg)
            }
...
function changeContactDutyOrder(tree_id, rtx_ids, direction){
    params = {
        'tree_id': tree_id,
        'rtx_ids': rtx_ids,
        'direction': direction
    ...
    $.ajax({
        url: 'node/association/change_duty_order',
        dataType: 'json',
        contentType: 'application/x-www-form-urlencoded',
        type: 'POST',
        data: params,
...
function editApprove(tree_id, rtx_ids, permission){
    params = {
        'tree_id': tree_id,
        'rtx_ids': rtx_ids,
    ...
    $.ajax({
        url: 'node/association/modify_contact_permission',
        dataType: 'json',
        contentType: 'application/x-www-form-urlencoded',
        type: 'POST',
        data: params,
...
function addContactDialog(node_id){
    var con_str = [];
    $("#add_contact_dialog").append('<input id="contact_list_select" style="width:260px">')
    $("#contact_list_select").autocomplete({
        source: function( request, response ) {
            var term = request.term.split(';');
            var term_len = term.length;
    ...
    title_html_str = '<div class="dialog_title">增加负责人</div>'
    var contact_dialog = $("#add_contact_dialog").removeClass('hide').dialog({
        modal: true,
        width: 300,
        resizable: true,
        title: title_html_str,
        title_html: true,
        position: { my: "center top", at: "center top", of: '#contact_panel'},
        buttons: [
            {
                text: "提交",
                "class" : "btn btn-primary btn-xs",
                click: function(){
                    var rtx_ids = $('#contact_list_select').val().replace(/(^[\s;]*)|([\s;]*$)/g, "").replace(/;/g, ',')
                    if(rtx_ids.length == 0){
                        showTips(2, '请输入负责人')
                        return
                    }
                    ...
                    $.ajax({
                        url: 'node/association/bind_contacts',
                        dataType: 'json',
                        contentType: "application/x-www-form-urlencoded",
                        type: 'POST',
                        data: params,
                        success: function(resp){
                            is_succ = resp.success
                            if(is_succ == true){
                                contact_dialog.dialog('close')
                                showContactsList(node_id, contact_p, super_p)
                                showTips(1, '绑定负责人成功')
                            }else{
                                showTips(2, resp.msg)
                            }
...
function showBindDevicesList(tree_id, bShowToolBar){
    bShowToolBar = (typeof(bShowToolBar)=='undefined') ? true: bShowToolBar
    now_time = new Date()
    $("#bind_device_panel").dataTablePanel({
        headerName: '已关联主机列表',
        bFull: true,
        rest_h: 146,
        devi: 6,
        ...
        aoColumns: [
            ...
            {"sTitle": "主机名称"},
            {"sTitle": "类型", "sWidth": "100px"},
            {"sTitle": "状态", "sWidth": "100px"},
            {"sTitle": "用途", "sWidth": "60px"}
        ],
        aDataSet: {tree_id: tree_id},
        fieldColcumns: ['id', 'name', 'type', 'status', 'usage'],
        ajax_url: "node/association/get_device_list?_dc=" + now_time.getTime(),
        method: 'GET',
        bShowToolBar: bShowToolBar,
        toolBar: [{
            id: 'remove_device',
            ...
            clickFn: function(otable, rowDatas, otherParams){
                ...
                if(row_datas_len == 0){
                    ...
                    var dev_id_list = []
                    for(var i=0; i<row_datas_len; i++){
                        dev_id_list.push(rowDatas[i][0])
                    }
                    var dev_ids = dev_id_list.join(',')
                    unbindDevice(otherParams.tree_id, dev_ids)
                }
            }
        },{
            id: 'select_all_bind_devices',
            name: '<i class="icon-stop"></i>',
            type: 'primary',
            floated: 'left',
            title: "全选 / 全不选",
            clickFn: function(otable, rowDatas, otherParams){
                if($("#select_all_bind_devices").find('.icon-stop').length==0){
                    otable.$("tr").each(function(){
                        $(this).removeClass('row_selected'); 
                    })
                    $("#select_all_bind_devices").html('<i class="icon-stop"></i>')
                }else{
                    otable.$("tr").each(function(){
                        $(this).addClass('row_selected'); 
                    })
                    $("#select_all_bind_devices").html('<i class="icon-check"></i>')
                }
            }
        }]
    })
...
function unbindDevice(tree_id, dev_ids){
    params = {
        'tree_id': tree_id,
        'dev_ids': dev_ids
    }
    $.ajax({
        url: 'node/association/unbind_devices',
        dataType: 'json',
        contentType: 'application/x-www-form-urlencoded',
        type: 'POST',
        data: params,
        success: function(resp){
            var is_succ = resp.success
            if(is_succ == true){
                showBindDevicesList(tree_id, device_p)
                showUnbindDevicesList(tree_id, false, device_p)
                showTips(1, '解除主机绑定成功')
            }else{
                showTips(2, resp.msg)
            }
...
function showUnbindDevicesList(tree_id, bDataCache, bShowToolBar){
    bShowToolBar = (typeof(bShowToolBar)=='undefined') ? true: bShowToolBar
    bDataCache = bDataCache ? bDataCache: false
    now_time = new Date()
    $("#unbind_device_panel").dataTablePanel({
        headerName: '未绑定主机列表',
        bFull: true,
        rest_h: 146,
        devi: 6,
        ...
        aDataSet: {tree_id: tree_id},
        fieldColcumns: ['id', 'name', 'type', 'status', 'usage'],
        ajax_url: "node/association/get_unbind_device_list?_dc=" + now_time.getTime(),
        method: 'GET',
        bDataCache: bDataCache,
        bShowToolBar: bShowToolBar,
        toolBar: [{
            id: 'add_device',
            name: '增加主机绑定',
            ...
            clickFn: function(otable, rowDatas, otherParams){
                ...
                if(row_datas_len == 0){
                    ...
                    var dev_ids = dev_id_list.join(',')
                    bindDevices(otherParams.tree_id, dev_ids)
                }
            }
        },{
            id: 'select_all_unbind_devices',
            name: '<i class="icon-stop"></i>',
            type: 'primary',
            floated: 'left',
            title: "全选 / 全不选",
            clickFn: function(otable, rowDatas, otherParams){
                if($("#select_all_unbind_devices").find('.icon-stop').length==0){
                    otable.$("tr").each(function(){
                        $(this).removeClass('row_selected'); 
                    })
                    $("#select_all_unbind_devices").html('<i class="icon-stop"></i>')
                }else{
                    otable.$("tr").each(function(){
                        $(this).addClass('row_selected'); 
                    })
                    $("#select_all_unbind_devices").html('<i class="icon-check"></i>')
                }
            }
        }]
    })
...
function bindDevices(tree_id, device_ids){
    params = {
        'tree_id': tree_id,
        'dev_ids': device_ids
    }
    $.ajax({
        url: 'node/association/bind_devices',
        dataType: 'json',
        contentType: 'application/x-www-form-urlencoded',
        type: 'POST',
        data: params,
...
function createTreeNodeNameFormValidate(form){
    var validator = form.validate({
        errorClass: 'error-label',
        focusInvalid: false,
        rules: {
            app_name_in: {required: true, checkTreeName: true},
            rtx_ids: {required: true}
        },
        messages: {
            app_name_in: {
                required: "节点名称不能为空"
        ...
        errorPlacement: function (error, element) {
            ...
            element.parent().next().css({"text-indent": "100px", "margin-top": "0px", "color": "red", "font-size": "12px"})
        }
    })
    return validator
}
